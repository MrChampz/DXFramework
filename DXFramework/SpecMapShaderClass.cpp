...
bool SpecMapShaderClass::Initialize(ID3D10Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"Shaders/SpecMap.vs", L"Shaders/SpecMap.ps");
	if (!result)
	{
		return false;
	}
...
bool SpecMapShaderClass::Render(ID3D10Device* device, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	XMMATRIX projectionMatrix, ID3D10ShaderResourceView** textureArray, XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor,
	XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, float specularPower)
{
	bool result;

	// Set the shader parameters that it will use for rendering
	result = SetShaderParameters(device, worldMatrix, viewMatrix, projectionMatrix, textureArray,
...
bool SpecMapShaderClass::InitializeShader(ID3D10Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	result = D3DCompileFromFile(vsFilename, NULL, NULL, "SpecMapVertexShader", "vs_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	result = D3DCompileFromFile(psFilename, NULL, NULL, "SpecMapPixelShader", "ps_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
...
bool SpecMapShaderClass::SetShaderParameters(ID3D10Device* device, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	XMMATRIX projectionMatrix, ID3D10ShaderResourceView** textureArray, XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor,
	XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, float specularPower)
{
	HRESULT result;
	...
	device->PSSetShaderResources(0, 3, textureArray);
