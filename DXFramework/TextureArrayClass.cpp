	...
	char* filename1, char* filename2, char* filename3)
{
	...
	hResult = device->CreateShaderResourceView(m_texture, &srvDesc, &m_textureView[0]);
	if (FAILED(hResult))
	{
		return false;
	}
	...
	// Create the shader resource view for the texture
	hResult = device->CreateShaderResourceView(m_texture, &srvDesc, &m_textureView[1]);
	if (FAILED(hResult))
	{
		return false;
	}
...
void TextureArrayClass::Shutdown()
{
	...
	// Release the first texture view resource
	if (m_textureView[0])
	{
