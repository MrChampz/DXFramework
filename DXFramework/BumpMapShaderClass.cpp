...
bool BumpMapShaderClass::Initialize(ID3D10Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"Shaders/BumpMap.vs", L"Shaders/BumpMap.ps");
	if (!result)
	{
		return false;
	}
...
bool BumpMapShaderClass::Render(ID3D10Device* device, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	XMMATRIX projectionMatrix, ID3D10ShaderResourceView** textureArray, XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor)
{
	bool result;

	// Set the shader parameters that it will use for rendering
	result = SetShaderParameters(device, worldMatrix, viewMatrix, projectionMatrix, textureArray, lightDirection, diffuseColor);
	if (!result)
	{
		return false;
	}
...
bool BumpMapShaderClass::InitializeShader(ID3D10Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	result = D3DCompileFromFile(vsFilename, NULL, NULL, "BumpMapVertexShader", "vs_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	result = D3DCompileFromFile(psFilename, NULL, NULL, "BumpMapPixelShader", "ps_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
...
bool BumpMapShaderClass::SetShaderParameters(ID3D10Device* device, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	XMMATRIX projectionMatrix, ID3D10ShaderResourceView** textureArray, XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor)
{
	HRESULT result;
	void* mappedResource;
	...
	device->PSSetShaderResources(0, 2, textureArray);
