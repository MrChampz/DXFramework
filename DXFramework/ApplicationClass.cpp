...
bool ApplicationClass::Initialize(HINSTANCE hInstance, HWND hwnd, int screenWidth, int screenHeight)
{
	...
	result = m_TextureManager->LoadTexture(m_Direct3D->GetDevice(), "Data/Maps/Dirt_diff.tga", 0);
	if (!result)
	{
		return false;
	}

	result = m_TextureManager->LoadTexture(m_Direct3D->GetDevice(), "Data/Maps/Dirt_normal.tga", 1);
	if (!result)
	{
		return false;
	}
	...
	result = m_Zone->Initialize(m_Direct3D, hwnd, screenWidth, screenHeight, SCREEN_DEPTH, SCREEN_NEAR);
	if (!result)
	{
